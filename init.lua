...
local search_fn = function()
	require("telescope.builtin").find_files({
		find_command = { "rg", "--files", "--hidden", "-g", "!.git", "-g", "!node_modules", "-u" },
	})
end
...
vim.keymap.set(
	...
	{ desc = "[R]esume [S]earch for previous picker" }
)
...
local on_attach = function(_, bufnr)
	...
	nmap("gr", function()
		return require("telescope.builtin").lsp_references({ include_current_line = true })
	end, "[G]oto [R]eferences")
	nmap("gI", require("telescope.builtin").lsp_implementations, "[G]oto [I]mplementation")
	nmap("gs", vim.lsp.buf.signature_help, "[G]imme [S]ignature")
